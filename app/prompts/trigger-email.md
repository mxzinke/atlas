...
The reply is delivered via SMTP with proper `In-Reply-To` and `References` headers — the email thread is maintained automatically. The subject line (`Re: ...`) and recipient are handled by the system. **Only provide the reply body**, not headers.
...
- `reply_send` — Send your reply via SMTP (threading handled automatically)
